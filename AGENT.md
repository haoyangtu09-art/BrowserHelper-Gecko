...
chats: List<SavedChat> / currentChatId   // 抽屉「最近」完整会话快照（空→只剩标签）
...
ensureCurrentChat() / replaceCurrentTitle() / saveCurrentChat() / loadChat() // 最近列表：先本地临时，再一次性 AI 替换，可点击恢复完整对话
...
- **#31 收起→展开对话丢失 + 无历史记录**
  ...
  - 历史记录：旧版 `recentChats` 一直空；现在新会话开始后创建 `SavedChat`，并保存完整 `messages/convo` 快照（标题见 #34）。
...
- **#34 按会话 AI 生成标题**：已改为首条真实用户消息先本地生成临时标题并创建 `SavedChat`，模型再根据前一到两句用户输入只做一次轻量命名替换；后续不实时更新。
- ~~楷书字体~~：用户已**取消**，不做。
