...
> 最新补齐：长期记忆已形成闭环。`memorySummary` + `memories` 持久化；记忆页可直接编辑摘要和条目，并可点「生成摘要」让模型按需压缩已有记忆；每次有效对话结束后 `AgentEngine.maybeExtractMemories()` 只轻量抽取新增长期记忆条目，临时聊天或关闭记忆时不写入。会话标题凑够前两句真实用户输入后只生成一次，不实时更新。
